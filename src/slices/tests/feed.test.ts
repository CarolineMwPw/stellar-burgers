...
import { feedSlice, initialState, getAllFeed } from '../feedSlice';

describe('test reducer feedSlice', function () {
  test('call pending', function () {
    const newState = feedSlice.reducer(initialState, getAllFeed.pending(''));
  ...
  test('call fulfilled', function () {
    ...
    const newState = feedSlice.reducer(
      ...
      getAllFeed.fulfilled(feedResponse, '')
    );
  ...
  test('call rejected', function () {
    ...
    const newState = feedSlice.reducer(
      initialState,
      getAllFeed.rejected(new Error(errorMessage), '')
    );
